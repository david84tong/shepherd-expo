...
export const StreakScreen = () => {
  ...
  const createdDate = useMemo(() => 
    createdAt
      ? dayjs((createdAt as any)?.toDate?.() ?? createdAt).startOf('day')
      : today,
    [createdAt, today]
  );
  ...
  const { completedSet, completedDates } = useMemo(() => {
    ...
    const dates = completedReadings.map(r => {
      const d = r.date;
      const formattedDate = d && typeof d.toDate === 'function'
  ...
  const [riveAssets] = useAssets([
    require('../assets/riveAnimations/Streak.riv')
  ]);
    ...
    <Animated.View style={containerStyle} className="flex-1 bg-surfaceCream justify-between">
      ...
      <Animated.View style={flameContainerStyle} className="items-center mt-10 mb-2">
        ...
        <View className="flex flex-col justify-center items-center -mt-20">
          <Animated.Text style={streakNumberStyle} className="absolute text-[96px] font-feather-bold text-textPrimary mb-12 -top-16">{streak}</Animated.Text>
          <Animated.Text style={streakTextStyle} className="text-textPrimary text-2xl font-feather-bold mb-1 tracking-wide mt-12">day streak!</Animated.Text>
        </View>
      </Animated.View>
      ...
      {__DEV__ && debugInfo && (
        <View style={{position: 'absolute', bottom: 80, left: 10, right: 10, backgroundColor: 'rgba(0,0,0,0.7)', padding: 8, borderRadius: 5}}>
          <Text style={{color: 'white', fontSize: 10}}>
            Streak: {streak} | Dates completed: {debugInfo.completedDates?.join(', ')}
          </Text>
          <Text style={{color: 'white', fontSize: 10}}>
            Today: {debugInfo.today} | Yesterday: {debugInfo.yesterday} | 2 Days Ago: {debugInfo.twoDaysAgo}
          </Text>
        </View>
      )}
    </Animated.View>
