...
import PrayerView, { PrayerViewRef } from '~/components/WaterPrayerView';
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (showDevotional === 'true') {
      ...
      // Set Rive to Reading state (9) when coming from LoadingScreen
      ...
      if (riveRef.current?.setInputState) {
        ...
        console.log('Set Rive Action-Number: 9 (Reading) from LoadingScreen');
      }
    }
  }, [showDevotional]);
      ...
      setTimeout(() => {
        if (riveRef.current && riveRef.current.setInputState) {
          try {
            ...
            // If coming from LoadingScreen with showDevotional=true, set reading state
            if (showDevotional === 'true' && showDevotionalContent) {
              console.log('Setting reading state from LoadingScreen in handleRivePlay');
              riveRef.current.setInputState('State Machine 1', 'Action-Number', 9);
              setCurrentStateInput(9);
              // Fallback for older artboards
              try {
                riveRef.current.setInputState('State Machine 1', 'Number 1', 9);
              } catch (_) {
                /* no-op – some artboards may not have this legacy input */
              }
              console.log('Set Rive Action-Number: 9 (Reading) from LoadingScreen in handleRivePlay');
            }
