...
const useSubscriptionStore = create<SubscriptionState>((set, get) => ({
  ...
  presentPaywall: async () => {
    ...
    try {
      ...
      view.registerEventHandlers({
        ...
        onProductSelected() {console.log("===>product selected");
         },
        onPurchaseStarted() {console.log("===>purrchase started") },
        onPurchaseCancelled() { console.log('cancelled')
        },
        onPurchaseFailed() { /* ... */ },
        onRestoreFailed() { /* ... */ },
        onRenderingFailed() { /* ... */ },
  ...
  getCustomerInfo: async () => {
    ...
    try {
      ...
      console.log('[SubscriptionStore] Adapty profile fetched successfully:', JSON.stringify(profile, null, 2));
      console.log('[SubscriptionStore] Adapty accessLevels:', profile.accessLevels);
      const isProAdapty = profile.accessLevels && profile.accessLevels['premium']?.isActive || false;
        ...
        try {
          ...
          if (userData) {
            ...
            if (userData.proExpiryDate) {
              ...
              } else if (typeof userData.proExpiryDate === 'string' || typeof userData.proExpiryDate === 'number') {
      ...
      console.log('[SubscriptionStore] Customer info and pro status updated in store (Adapty + Firestore).');
    } catch (e) {
